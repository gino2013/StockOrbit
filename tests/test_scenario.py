...
def demo():
    ...
    assert result["uncovered_symbols"] == []  # every symbol had a computable beta
    ...
    # missing beta -> None estimate, not a crash or a bogus 0, and flagged
    # in uncovered_symbols so the caller can disclose the understatement
    # instead of silently implying beta=0 for those symbols.
    with patch.object(scenario, "compute_risk_metrics", return_value=[]):
        result = scenario.simulate_market_drop(snapshots, -0.10)
    by_symbol = {r["symbol"]: r for r in result["items"]}
    assert by_symbol["HIBETA"]["estimated_change"] is None
    assert sorted(result["uncovered_symbols"]) == ["HIBETA", "LOBETA"]
