...
輸入年支出跟安全提領率（預設 4% 法則），算出財務獨立所需的市值（FIRE 數字 = 年支出 ÷ 提領率），進度條顯示目前市值 vs FIRE 數字，還差多少、以目前 XIRR 推估的達成日期一次看完。
...
- **FIRE 進度**：輸入年支出跟安全提領率（預設 4% 法則），算出財務獨立所需的市值（FIRE 數字 = 年支出 ÷ 提領率），追蹤目前進度、還差多少、以目前 XIRR 推估的達成日期
