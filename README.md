...
- 修正「近期市場動態」會把合成的 `CASH` 現金列誤當成真實股票代號查詢（跟 Nasdaq 上市代號 CASH 撞名），導致顯示不相關公司新聞的 bug
- CI/圖示：favicon 與頁首圖示換成實際的柴犬圖片（自備圖片，壓縮到 128px 內嵌），拿掉不必要的白底圈
- 正式接上 Neon Postgres；USD/TWD 匯率改成每次「重新抓取持股」時存一筆快照（`exchange_rate_snapshots`），首頁改讀資料庫最新一筆，不再每次載入頁面都即時查匯率
- 再平衡回測改成一進頁面就自動載入（前提是已設定目標配置），跟持股歷史走勢一致
