...
![蒙地卡羅模擬：歷史報酬隨機重抽的信賴區間扇形圖](docs/screenshot-monte-carlo.png)

「接下來預測」是假設固定報酬率的單一路徑；這裡改用歷史每日報酬隨機重抽（bootstrap，不假設常態分布），模擬大量條可能路徑，畫出 10%／50%／90% 分位數的區間，講清楚「這只是眾多可能之一」，一樣不是預測。可自訂模擬月數。
...
![批次比較（Tax Lot）：FIFO vs HIFO 賣出比較](docs/screenshot-tax-lots.png)

已實現損益固定用 FIFO（最舊批次先賣）配對，但賣出時如果能挑「特定批次」，同一筆賣出的已實現損益可能差很多。輸入標的跟股數，假設「現在」用目前市價賣出，比較 FIFO vs HIFO（挑成本最高的批次先賣，實現損益最小）兩種挑法的差異，並列出目前持有的所有未賣批次（買進日期／股數／成本）。純試算，不代表 Firstrade 實際支援指定批次賣出。
...
![資金流 Sankey 圖：入金/股利最後去了提領還是留在帳戶](docs/screenshot-cash-flow-sankey.png)

入金＋股利收入最後去了哪裡——提領出去，還是留在帳戶（現金＋投資）。跟「目前市值」不同（不含未實現投資損益）。提領無法追蹤是哪一筆錢，用「先花股利、再動本金」的心態模型分配，明確標示是簡化假設，不代表真實資金流向。
...
![價格提醒：設定目標價，漲過/跌破會排程寄信通知](docs/screenshot-price-alerts.png)

針對任一標的設一個目標價（漲過／跌破），排程 job 每小時檢查一次，觸發時寄一封信到你註冊的信箱；同一次觸發只通知一次，不會每小時重複寄，已觸發的提醒要自己刪除後重新設定，不會自動重置。跟每日摘要 Email 共用同一組 `SMTP_*` secrets 才會真的寄出。
...
![美債殖利率曲線：13週/5年/10年/30年殖利率目前水準](docs/screenshot-yield-curve.png)

美國公債殖利率（13 週／5 年／10 年／30 年）目前水準，可選跟 N 個月前疊圖比較，短天期利率高於長天期（倒掛）時會有提示。純資訊呈現，不是預測。
...
![機構持股／內部人交易：主要機構持股人跟最近內部人申報交易](docs/screenshot-institutional-holders.png)

查任一股票代碼，看主要機構持股人清單（持股比例／股數／申報日）跟最近的內部人（公司高層）申報交易，純資訊呈現，不做買賣建議。ETF 通常沒有這類資料；即時抓取，沒有排程快取，Render 連不到 Yahoo 時顯示查無資料。
...
![持股風格箱：規模 x 風格九宮格，市值加權配置熱力圖](docs/screenshot-style-box.png)

市值加權配置在「規模（大型/中型/小型）× 風格（價值/平衡/成長）」九宮格的哪裡，一眼看出整體持股的風格傾向。分類用固定門檻（市值 100 億/20 億美元；本益比 15/25，缺本益比才退回股價淨值比 2/4），不是跟持股自己比排名，也不是投資建議。ETF 橫跨多種風格、缺資料的標的都不勉強塞進格子，另外列在「未分類」。

#### 效率前緣

![效率前緣：Markowitz 均值變異數分析，隨機權重組合的風險-報酬散點圖](docs/screenshot-efficient-frontier.png)

用持股的歷史報酬／波動度隨機取樣大量長期持有／不做空／不槓桿的權重組合，畫出風險（年化波動度）－報酬（年化報酬率）散點圖，藍點是目前實際配置的位置。純資訊呈現，不是「應該調整成前緣上哪一點」的投資建議。
...
![績效歸因：拆解目前配置 vs 目標配置的報酬差異](docs/screenshot-attribution.png)

拆解「目前持股 vs 你自己設定的目標配置」在某段期間的報酬差異，有多少是因為某檔標的目前佔比偏離目標（超配/低配）。基準是你自己的目標配置，不是大盤——跟大盤比沒有「配置」這個維度可比。因為比較的兩邊是同一批標的，只有權重不同，選股效果／交互效果在數學上恆為 0，全部差異都會落在配置效果上；正值代表偏離目標的配置目前是加分的，負值代表扣分。純資訊呈現，不建議你該不該調整。
...
- **價格提醒**：針對任一標的設目標價（漲過／跌破），GitHub Actions 排程每小時檢查一次，觸發時寄信到註冊信箱；同一次觸發只通知一次，已觸發的提醒不會自動重置，要自己刪除後重新設定
...
- **持股風格箱**：市值加權配置在「規模（大型/中型/小型）× 風格（價值/平衡/成長）」九宮格的哪裡，一眼看出整體持股的風格傾向。分類用固定門檻（市值、本益比，缺本益比退回股價淨值比），不跟持股自己比排名；ETF 跟缺資料的標的不勉強分類，另外列出
...
- **績效歸因（配置效果）**：拆解目前持股 vs 目標配置在某段期間的報酬差異，看有多少來自超配/低配某檔標的。基準是你自己的目標配置而非大盤；因為比較的是同一批標的，選股效果／交互效果數學上恆為 0，差異全部歸在配置效果
...
app/
  ...
  domain/
    ...
    notifications/
      ...
      price_alerts.py            # 價格提醒觸發判斷（純函式，含去重）
  infrastructure/
  ...
  templates/
    ...
    sections/                    # 39 個功能區塊 partial（HTML + 對應 JS），_shared.html 放跨區塊共用工具
...
- 新增蒙地卡羅模擬、效率前緣（Markowitz）、批次比較（Tax Lot：FIFO vs HIFO）、美債殖利率曲線、機構持股／內部人交易查詢、資金流 Sankey 圖；稅務效率分析補上 wash sale（洗售）偵測；頁首新增主題切換（daisyUI 全部 35 種主題）
