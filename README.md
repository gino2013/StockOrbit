...
- **個股資訊查詢**（`/stock/{代碼}`）：輸入任一股票代碼，重現 Google Finance 個股頁的資訊——名稱/交易所、目前價格、依所選區間（1天/5天/1個月/6個月/本年迄今/1年/5年/最久）計算的漲跌幅走勢圖，以及開盤/市值/本益比/最高/股息/季度股利金額/最低/52 週高低點。從側邊選單或持股表的代號點進去，頁面上也有「回到上一頁」跟換代碼查詢的輸入框。Render 連不到 Yahoo 即時 API 時走勢圖仍正常（不同的 yfinance 端點），指標欄位會顯示「-」
