...
輸入年支出跟安全提領率（預設 4% 法則），算出財務獨立所需的市值（FIRE 數字 = 年支出 ÷ 提領率），進度條顯示目前市值 vs FIRE 數字，還差多少、以目前 XIRR 推估的達成日期一次看完。另外填退休日期跟預期實質報酬率可以算 **Coast FIRE**：今天起不再投入、純靠複利，能不能長到 FIRE 數字，算出「今天需要有多少才能躺平」跟還差多少。
...
- **FIRE 進度**：輸入年支出跟安全提領率（預設 4% 法則），算出財務獨立所需的市值（FIRE 數字 = 年支出 ÷ 提領率），追蹤目前進度、還差多少、以目前 XIRR 推估的達成日期；另外填退休日期跟預期實質報酬率可以算 Coast FIRE（今天起不再投入，純靠複利能否長到 FIRE 數字）
