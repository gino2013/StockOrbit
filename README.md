...
![FIRE 進度：4% 法則進度、Coast FIRE、股利覆蓋率三個角度](docs/screenshot-fire.png)

輸入年支出跟安全提領率（預設 4% 法則），算出財務獨立所需的市值（FIRE 數字 = 年支出 ÷ 提領率），進度條顯示目前市值 vs FIRE 數字，還差多少、以目前 XIRR 推估的達成日期一次看完。另外填退休日期跟預期實質報酬率可以算 **Coast FIRE**：今天起不再投入、純靠複利，能不能長到 FIRE 數字，算出「今天需要有多少才能躺平」跟還差多少。下方還有**股利覆蓋率**（Barista / Dividend FIRE）：近 12 個月股利覆蓋了多少年支出，以目前殖利率回推全額覆蓋大概還需要多少本金——不用另外設定，自動用既有股利紀錄算。
...
找出目前有未實現虧損的部位，估算賣出這些部位可以抵銷多少今年已達起課的海外所得、大概能省多少稅。估計金額會正確地被「今年海外所得是否已超過免稅額」這個前提卡住，超過才會顯示實際數字，不會憑空誇大節稅效果。會檢查 wash sale（洗售）規則：最近 30 天內買過的標的，今天賣出虧損不能抵稅，這種候選會標示出來、不計入估計節稅金額，也會提醒賣出後 30 天內不要買回同一標的。純規則式提示，不建議賣出時機。
...
![個股資訊查詢：重現 Google Finance 個股頁的價格走勢與關鍵指標](docs/screenshot-stock-lookup.png)

輸入任一股票代碼，重現 Google Finance 個股頁的資訊：名稱/交易所、目前價格、依所選區間（1天/5天/1個月/6個月/本年迄今/1年/5年/最久）計算的漲跌幅走勢圖，以及開盤/市值/本益比/最高/股息/季度股利金額/最低/52 週高低點。是「美元匯率歷史」下面常駐的一個區塊，從側邊選單或持股表的代號點過去都是捲動＋自動查詢。ETF 沒有市值時退回顯示資產規模，股息欄位遇到 yfinance 常見缺值的 ETF 時退回顯示近 12 個月實際配發總額。
...
- **持股同步**：透過非官方 Firstrade API 自動登入（支援 TOTP 兩步驟驗證），抓取目前帳戶持倉（含未投入的現金）並存成歷史快照。同一登入下有多個帳戶時，同代號自動加總成一列，不會重複列出
- **帳戶篩選**：多帳戶時頁首會出現下拉選單，可選「全部帳戶」或只看單一帳戶，總市值/持股表格/圓餅圖/目標配置比對/再平衡建議/已實現損益/股利追蹤/目標達成進度/FIRE 進度/閒置現金建議/月度年度績效報告都會跟著只算該帳戶的資料；只有一個帳戶時選單不出現，跟以前一樣
...
- **蒙地卡羅模擬**：「接下來預測」是單一固定報酬率路徑，這裡改用歷史每日報酬隨機重抽（bootstrap，不假設常態分布），模擬大量條可能路徑，畫出 10%／50%／90% 分位數區間的扇形圖，講清楚這只是眾多可能之一，可自訂模擬月數
- **FIRE 進度**：輸入年支出跟安全提領率（預設 4% 法則），算出財務獨立所需的市值（FIRE 數字 = 年支出 ÷ 提領率），追蹤目前進度、還差多少、以目前 XIRR 推估的達成日期；另外填退休日期跟預期實質報酬率可以算 Coast FIRE（今天起不再投入，純靠複利能否長到 FIRE 數字）；再加上股利覆蓋率（Barista / Dividend FIRE）——近 12 個月股利覆蓋多少年支出、全額覆蓋大概還需要多少本金；以及儲蓄率——近 12 個月淨入金 ÷（淨入金＋年支出），有填預期實質報酬率的話再推估這個儲蓄率下大概還要幾年財務自由（假設較多、僅供參考，淨入金不等於真正的儲蓄）
- **目標達成進度追蹤**：設定長期目標金額與日期，追蹤目前進度、達標所需的年化報酬率，跟目前實際 XIRR 對照。推估市值/達標日期會把你過去的定期入金習慣（依歷史 DEPOSIT 推算的年入金額）一起算進去，不是只靠投資報酬
- **再平衡建議金額**：依目前總市值跟目標配置反推每檔要加碼/減碼多少錢才能貼齊目標；也支援「只有一筆閒置現金」情境，算出怎麼分配最接近目標
- **已實現損益**：FIFO（先進先出）配對買賣紀錄，列出每筆已實現損益明細跟年度彙總
- **批次比較（Tax Lot）**：假設現在用目前市價賣出指定股數，比較 FIFO（最舊批次先賣，已實現損益計算固定用的方式）vs HIFO（最高成本批次先賣，實現損益最小）兩種挑法的差異，列出目前持有的所有未賣批次（買進日期/股數/成本）。純試算，不代表 Firstrade 實際支援指定批次賣出
- **海外所得試算**：已實現利得＋股利換算台幣，粗估是否達到海外所得申報／課稅門檻（僅供參考，非正式稅務建議）
- **稅務效率分析**：找出目前未實現虧損的部位，估算賣出可以抵銷多少今年已達起課的海外所得、大概能省多少稅（純規則式提示，不建議賣出時機）；會檢查 wash sale（洗售）規則，最近 30 天內買過的標的今天賣出不能抵稅，會標示出來並排除在估計節稅金額外
- **股利追蹤**：近 12 個月各標的股利收入彙總與殖利率
- **資金流 Sankey 圖**：入金＋股利收入最後去了哪裡——提領出去或留在帳戶（現金＋投資），跟「目前市值」不同（不含未實現投資損益）。提領無法追蹤是哪一筆錢，用「先花股利、再動本金」的心態模型分配，明確標示是簡化假設，不代表真實資金流向
- **配息月曆**：依歷史股利發放的月份規律，推算未來 12 個月的預期配息時間與金額
- **持股筆記**：每檔標的可以記錄當初買進理由、目標價等提醒文字，唯讀/編輯兩種顯示模式；每次存檔會另外留一筆帶時間戳記的歷史版本，筆記下方「歷史版本」摺疊列表由新到舊列出

### 市場資訊

- **持股健康度總覽**：整合風險/相關性/Beta 等區塊的關鍵數字成一張卡片，另外算出 Sharpe、Sortino、Calmar 三個風險調整後報酬比率（年化報酬用 XIRR，無風險利率用固定假設值），一次看完「報酬相對於承擔的風險划不划算」；再加上歷史模擬法的單日 VaR / CVaR（95% 信心水準），美元金額跟百分比並列
...
- **美債殖利率曲線**：13 週/5 年/10 年/30 年殖利率目前水準（yfinance `^IRX`/`^FVX`/`^TNX`/`^TYX`），可選跟 N 個月前疊圖比較，短天期利率高於長天期（倒掛）時會有提示。純資訊呈現，不是預測
- **機構持股／內部人交易**：查任一股票代碼，看主要機構持股人清單跟最近的內部人（公司高層）申報交易，純資訊呈現，不做買賣建議。ETF 通常沒有這類資料；即時抓取，沒有排程快取，Render 連不到 Yahoo 時顯示查無資料
- **個股資訊查詢**：輸入任一股票代碼，重現 Google Finance 個股頁的資訊——名稱/交易所、目前價格、依所選區間（1天/5天/1個月/6個月/本年迄今/1年/5年/最久）計算的漲跌幅走勢圖，以及開盤/市值（ETF 沒有市值時退回顯示資產規模）/本益比/最高/股息（ETF 常見的 yfinance 缺值時退回顯示近 12 個月實際配發總額）/季度股利金額/最低/52 週高低點。是「美元匯率歷史」下面常駐的一個區塊（不是另開頁面），從側邊選單或持股表的代號點過去都是捲動＋自動查詢。這些欄位（含股息）都跟其他基本面欄位一樣，走對 Render 連不到 Yahoo 即時 API 有快取退回的管道；開盤/當日高低/現價改用不會被擋的走勢資料算，所以永遠看得到。第一次查一個從沒持有過的冷門代號，會順便把代號登記進排程快取名單，並立刻觸發一次快取重新整理（設了 `GH_ACTIONS_TOKEN` 的話，見下方環境變數）；沒有這把 token，或觸發的那次剛好也被 Yahoo 擋，才會等到下次排程（最晚 6 小時）
...
- **效率前緣**：Markowitz 均值變異數分析，隨機取樣大量長期持有／不做空／不槓桿的權重組合，畫出風險-報酬散點圖，並標出目前實際配置的位置。純資訊呈現，不是「應該調成前緣上哪一點」的建議
...
- **儀表板**：單頁網頁介面，含深色模式，資料存在本機（開發）或 Postgres（正式環境）；側邊選單可以收合到最小，騰出畫面空間（記憶在瀏覽器 localStorage，重整後維持上次的收合狀態）；頁首（logo/匯出/使用者資訊）常駐置頂，往下捲動時也看得到
...
- 新增帳戶篩選：多帳戶時頁首出現下拉選單，可只看單一帳戶；單一帳戶時不出現，體驗不變
