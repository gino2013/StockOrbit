...
- **FIRE 進度**：輸入年支出跟安全提領率（預設 4% 法則），算出財務獨立所需的市值（FIRE 數字 = 年支出 ÷ 提領率），追蹤目前進度、還差多少、以目前 XIRR 推估的達成日期；另外填退休日期跟預期實質報酬率可以算 Coast FIRE（今天起不再投入，純靠複利能否長到 FIRE 數字）；再加上股利覆蓋率（Barista / Dividend FIRE）——近 12 個月股利覆蓋多少年支出、全額覆蓋大概還需要多少本金
