...
- `f9db3ad` flex 模式下「報酬率」改成「假設每檔現在的持股都在 2017-01-01 當天的價位買進」的價格報酬，用各持股目前市值加權——比原本的未實現報酬率大得多（原本是比率，×10.1 不會變）。晚於 2017 才上市的持股（例如 2023/2024 IPO）會用該檔第一個有資料的收盤價當基準，不會被跳過（實測 AAPL 抓到 ~$26.70、ARM 抓到 ~$32）。只有 flex cookie 開啟時才多打一次 yfinance；抓失敗就退回原本的報酬率，頁面不會壞。demo 組合實測：報酬率 +13.6% → +1355.6%
